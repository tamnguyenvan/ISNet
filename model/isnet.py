...
def muti_loss_fusion_kl(preds, target, dfs, fs, mode='MSE'):
    ...
    for i in range(0, len(dfs)):
        if (mode == 'MSE'):
            ...
            loss = loss + \
                kl_loss(F.log_softmax(dfs[i], dim=1), F.softmax(fs[i], dim=1))
...
class ISNetSmall(nn.Module):

    def __init__(self, in_ch=3, out_ch=1):
        super(ISNetSmall, self).__init__()

        self.conv_in = nn.Conv2d(in_ch, 64//4, 3, stride=2, padding=1)
        self.pool_in = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage1 = RSU7(64//4, 32//4, 64//4)
        self.pool12 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage2 = RSU6(64//4, 32//4, 128//4)
        self.pool23 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage3 = RSU5(128//4, 64//4, 256//4)
        self.pool34 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage4 = RSU4(256//4, 128//4, 512//4)
        self.pool45 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage5 = RSU4F(512//4, 256//4, 512//4)
        self.pool56 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage6 = RSU4F(512//4, 256//4, 512//4)

        # decoder
        self.stage5d = RSU4F(1024//4, 256//4, 512//4)
        self.stage4d = RSU4(1024//4, 128//4, 256//4)
        self.stage3d = RSU5(512//4, 64//4, 128//4)
        self.stage2d = RSU6(256//4, 32//4, 64//4)
        self.stage1d = RSU7(128//4, 16//4, 64//4)

        self.side1 = nn.Conv2d(64//4, out_ch, 3, padding=1)
        self.side2 = nn.Conv2d(64//4, out_ch, 3, padding=1)
        self.side3 = nn.Conv2d(128//4, out_ch, 3, padding=1)
        self.side4 = nn.Conv2d(256//4, out_ch, 3, padding=1)
        self.side5 = nn.Conv2d(512//4, out_ch, 3, padding=1)
        self.side6 = nn.Conv2d(512//4, out_ch, 3, padding=1)
...
class Trainer(pt.LightningModule):
    def __init__(self, in_ch, out_ch, args):
        ...
        if args.model_type == 'base':
            ...
            self.net = ISNetSmall(in_ch=in_ch, out_ch=out_ch)
        else:
            raise ValueError(f'Not supported model type: {args.model_type}')
        self.args = args
